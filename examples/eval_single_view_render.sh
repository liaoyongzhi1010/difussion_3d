...
mkdir -p "$OUTPUT_ROOT"
PYTHONPATH=src "$PYTHON_BIN" scripts/eval/eval_single_view_render_metrics.py \
  ...
  --split "$SPLIT" \
  --output-dir "$OUTPUT_ROOT/render_eval_${SPLIT}_p${NUM_POSTERIOR_SAMPLES}_s${NUM_INFERENCE_STEPS}" \
  --save-summary "$OUTPUT_ROOT/render_eval_${SPLIT}_p${NUM_POSTERIOR_SAMPLES}_s${NUM_INFERENCE_STEPS}.json"
